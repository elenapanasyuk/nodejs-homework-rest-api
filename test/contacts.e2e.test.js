...
describe("Testing the route api/contacts", () => {
  ...
  describe("Testing: get all contacts", () => {
    it("Get all contacts should return status 200", async () => {
    ...
    it("Get contact by id should return status 200", async () => {
    ...
    it("Get contact with wrong id should return 404 status", async () => {
      const wrongId = 12345;
      const res = await request(app)
  ...
  describe("Testing: post request for creating contact", () => {
    it("Create contact should return status 201", async () => {
      const res = await request(app)
        .post("/api/contacts")
        .set("Authorization", `Bearer ${token}`)
        .send(newContact)
    ...
    // it("Without required field name should return 400 status", async () => {
    //   const res = await request(app)
    //     .post("/api/contacts")
    //     .set("Authorization", `Bearer ${token}`)
    //     .send({ name: "Simon" })
  ...
  describe("Testing: patch request for updating contact", () => {
    it("Update contact should return status 200", async () => {
      const res = await request(app)
    ...
    it("Wrong id should return 404 status", async () => {
      const res = await request(app)
    ...
    it("Wrong subscription field should return 400 status", async () => {
      const res = await request(app)
        .patch(`/api/contacts/${idNewContact}`)
        .set("Authorization", `Bearer ${token}`)
  ...
  describe("Testing: remove contact", () => {
    it("Remove contact should return status 200", async () => {
      const res = await request(app)
